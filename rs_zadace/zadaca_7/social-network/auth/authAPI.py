...
async def register(request):
    try:
        ...
        if not korisnicko_ime or not lozinka:
            return web.json_response({"error": "Nedostaju podaci!"}, status=400)
        if any(k["korisnicko_ime"] == korisnicko_ime for k in korisnici):
            return web.json_response({"error": "Korisnik postoji!"}, status=409)
        lozinka_hash = hash_data(lozinka)
        korisnici.append(
            {"korisnicko_ime": korisnicko_ime, "lozinka_hash": lozinka_hash}
        )
        return web.json_response({"message": "Registriran"}, status=201)
    ...
    try:
        ...
        if not korisnik:
            return web.json_response(
                {"authorized": False, "error": "Korisnik ne postoji"}, status=404
            )

        if hash_data(lozinka) != korisnik["lozinka_hash"]:
            return web.json_response(
                {"authorized": False, "error": "Pogrešna lozinka"}, status=401
            )

        return web.json_response({"authorized": True, "message": "Uspjeh"}, status=200)
